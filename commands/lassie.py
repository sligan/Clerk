...
def new_lassie_day():
    ...
    client.chat_postMessage(channel=channel_id,
                            text='*Lassie Smoke – Day*\n'
                                 ...
                                 f'Bounce Rate: {round(bounce_rate,2)}% ({compare_br})\n'
                                 f'K-factor Rate (Viral): {k_factor_rate}% ({compare_k_factor})\n'
                                 '\n'
                                 '*Product* 🍏\n'
...
def new_lassie_week():
    ...
    client.chat_postMessage(channel=channel_id,
                            text='*Lassie Smoke – Week*\n'
                                 ...
                                 f'User Acquisition (UA): {new_users_landing} ({compare_nul})\n'
                                 f'Conversion to User (CR1): {conversion_to_user}% ({compare_cto})\n'
                                 f'Bounce Rate: {round(bounce_rate, 1)}% ({compare_br})\n'
                                 f'K-factor Rate (Viral): {k_factor_rate}% ({compare_k_factor})\n'
                                 '\n'
                                 '*Product* 🍏\n'
                                 f'New Users: {new_users} ({compare_nu})\n'
...
def lassie_all():
    ...
    new_users = new_users_db(calculations.start_day())
    new_users_landing = lassie_ga_metrics(startDate='2021-01-01', metrics="ga:newUsers")
    conversion_to_user = calculations.compare2_0(new_users, new_users_landing)
    bounce_rate = float(lassie_ga_metrics(startDate='2021-01-01', metrics="ga:bounceRate"))
    onboarding_rate = calculations.compare2_0(new_users_db_active(calculations.start_day()), new_users)
    aha_moment_rate = calculations.compare2_0(aha(calculations.start_day()), new_users)
    average_interval_increases_rate = calculations.compare2_0(interval(calculations.start_day()), new_users)
    active_users = count_active_users(calculations.start_day())
    k_factor_rate = calculations.compare2_0(lassie_ga_metrics(startDate='2021-01-01', metrics="ga:newUsers",
...
def lassie_metrics(start, end, higher_date, lower_date):
    ...
    compare_nul = calculations.compare(new_users_landing,
    ...
    compare_br = calculations.compare(bounce_rate,
                                      lassie_ga_metrics(startDate=start, metrics='ga:bounceRate', endDate=end))

    k_factor_rate = calculations.compare2_0(lassie_ga_metrics(startDate=end, metrics="ga:newUsers",
                                                              dimensions=[{'name': 'ga:referralPath'}]),
                                            new_users_landing)
    k_factor_rate_prev = calculations.compare2_0(lassie_ga_metrics(startDate=start, endDate=end, metrics="ga:newUsers",
                                                                   dimensions=[{'name': 'ga:referralPath'}]),
                                                 lassie_ga_metrics(startDate=start, metrics="ga:newUsers", endDate=end))

    compare_k_factor = calculations.compare(k_factor_rate, k_factor_rate_prev)

    conversion_to_user = calculations.compare2_0(new_users, new_users_landing)
    conversion_to_user_prev = calculations.compare2_0(new_users_db(higher_date, lower_date),
                                                      lassie_ga_metrics(startDate=start, metrics='ga:newUsers',
                                                                        endDate=end))
    compare_cto = calculations.compare(conversion_to_user, conversion_to_user_prev)

    aha_moment_rate = calculations.compare2_0(aha(lower_date), new_users)
    aha_moment_rate_prev = calculations.compare2_0(aha(higher_date, lower_date), new_users_db(higher_date, lower_date))
    compare_amr = calculations.compare(aha_moment_rate, aha_moment_rate_prev)

    onboarding_rate = calculations.compare2_0(new_users_db_active(lower_date), new_users)
    compare_or = calculations.compare(
        onboarding_rate, calculations.compare2_0(new_users_db_active(higher_date, lower_date),
                                                 new_users_db(higher_date, lower_date)))

    average_interval_increases_rate = calculations.compare2_0(interval(lower_date), new_users)
    average_interval_increases_rate_prev = calculations.compare2_0(interval(higher_date, lower_date),
                                                                   new_users_db(higher_date, lower_date))
    compare_ir = calculations.compare(average_interval_increases_rate, average_interval_increases_rate_prev)

    compare_cau = calculations.compare(count_active_users(lower_date), count_active_users(higher_date, lower_date))
