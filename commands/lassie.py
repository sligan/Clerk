...
from db_connect import lessie_get
...
def lessie_users_total():
    users = lessie_get("""SELECT count(id) FROM users""")
    data = request.form
...
def lassie_day():
    data = request.form
    channel_id = data.get('channel_id')
    count_new_users_id = lessie_get(query="SELECT count(distinct(id)) FROM users WHERE created_at > now() - interval "
                                          "'24 hours'")
    count_active_users = lessie_get(query="SELECT count(distinct(user_id)) FROM smoking_actions WHERE smoke_at > "
                                          "now() - interval '24 hours'")
    aha = lessie_get(query="SELECT count(distinct(smoke_at)), count(distinct(users.id)) as id_count FROM users join "
    ...
    ret = lessie_get(query="SELECT count(sa.smoke_at),count(distinct(users.id)) FROM users join smoking_actions sa "
    ...
    landing_conversion_rate = timestamp.compare2_0(lessie_ga_metrics(startDate="1daysAgo", metrics="ga:sessions"),
    ...
    client.chat_postMessage(channel=channel_id,
                            text="*Lassie Smoke*"
                                 ...
                                 f'\n Value: {lessie_ga_metrics(startDate="1daysAgo", metrics="ga:sessions")} \n'
                                 '\n Metric: Total Users'
                                 '\n Description: Суммарное количество пользователей'
                                 f'\n Value: {count_new_users_id.iloc[0]["count"]}\n' 
